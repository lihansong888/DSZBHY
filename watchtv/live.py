...
# HEAD/GET 粗筛会把大量国内可用源误杀（且并发2+重试2次又慢又耗Action额度）。
# 存活筛选全部交给宝塔端（国内、100并发、5~6秒阈值、一天2次），
# 本仓库只负责：拉取上游源 -> 分组映射 -> 合并去重 -> 输出全量 live.m3u8。
...
def main():
    ...
    print(f"✅采集完成，共提取 {total_cnt} 个频道（跳过粗筛检测，全量输出）")
